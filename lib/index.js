    ...
    _createClass(BloggifyPlugin, [{
        ...
        value: function _init(cb) {
            ...
            this._load(function (err, main) {
                ...
                if (_this4.config.init_theme !== false && Array.isArray(_this4.config.templates)) {
                    _this4.config.templates.forEach(function (c) {
                        var tmpl = Bloggify.renderer.registerTemplate(path.resolve(__dirname, "..", c));
                        tmpl.data = { theme: _this4.config.options };
                    });
